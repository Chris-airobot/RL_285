...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        #     vanilla += (self.gamma ** i)*rewards[i]


        vanillas = []
        for reward in rewards:
            gammas = np.ones(len(reward))*self.gamma

            gammas = np.array([gammas[i]**i for i in range(len(reward))])
            
            vanilla = np.sum(gammas*reward)
            vanilla = [vanilla]*len(reward)
            vanillas.append(vanilla)
        return vanillas
        ...
        # for reward in rewards:
        #     total_return = sum(
        #         [self.gamma**t * reward[t] for t in range(len(reward))]
        #     )

        #     # Create a list with the total_return value for each time step
        #     discounted_returns = [total_return] * len(reward)
        #     vanillas.append(discounted_returns)
        # return vanillas
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        final = []
        for reward in rewards:
            reward_to_gos = [0]*len(reward)
            for t in range(len(reward)):
                reward_to_go = 0
                for t_prime in range(t, len(reward)):
                    reward_to_go += self.gamma ** (t_prime - t) * reward[t_prime]
                reward_to_gos[t] = reward_to_go
            final.append(reward_to_gos)
        return final