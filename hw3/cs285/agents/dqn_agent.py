...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.02) -> int:
        ...
        if np.random.rand() > epsilon:
            ...
            action = torch.randint(0, self.num_actions,(1,1)).to(ptu.device)
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                next_action = torch.argmax(self.critic(obs),dim=1)
            else:
            ...
            # except:
            #     print(f'next_qa dim is:{next_qa_values.size()}')
            #     print(f'next_action dim is:{next_action.unsqueeze(1).size()}')
            #     quit()
